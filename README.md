...
Formatted, versioned legislative data for Illinois. Part of the [govbot](https://github.com/chihacknight/govbot) civic data project.
...
This is the **formatted data repo** for Illinois — one of two repos per jurisdiction:

1. **Scraper repo** (`govbot-openstates-scrapers/il-legislation`) — runs [OpenStates](https://github.com/openstates/openstates-scrapers) scrapers, produces raw scraped JSON
2. **This repo** (`il-legislation`) — reads the scraper repo's output and:
...
To change Illinois's configuration (branch pinning, runner, enabled jobs, labels), edit its entry in [`chn-openstates-files.yml`](https://github.com/chihacknight/govbot/blob/main/actions/pipeline-manager/chn-openstates-files.yml) and re-run `apply.py` — don't hand-edit the workflow files in this repo directly, since the next `apply.py` run will overwrite them.

Current settings for Illinois:

- **State code**: `il`
- **Toolkit branch**: `main`
- **Scraper repo**: `govbot-openstates-scrapers/il-legislation`
...
Pulls the latest scraped data from the scraper repo, sanitizes it, formats it into `country:us/state:il/sessions/...`, links events, and updates orphan tracking.
...
├── country:us/
│   └── state:il/            # state:usa for federal, state:il for Illinois, etc.
│       └── sessions/
│           └── {session_id}/
│               ├── bills/
│               │   └── {bill_id}/
...
Formatted metadata is saved to `country:us/state:il/sessions/`, organized by session and bill.
...
- Check the scraper repo (`govbot-openstates-scrapers/il-legislation`) actually got fresh data first — this repo only formats what that repo has already scraped
